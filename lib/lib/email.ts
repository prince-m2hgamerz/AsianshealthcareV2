...
const from = process.env.RESEND_FROM_EMAIL || `"${DEFAULT_SETTINGS.site_name}" <noreply@${new URL(DEFAULT_SETTINGS.site_url).hostname}>`;
const fallbackAdminEmail = process.env.ADMIN_EMAIL || DEFAULT_SETTINGS.admin_email;
...
async function getWhatsAppNumber(): Promise<string> {
  ...
  return process.env.NEXT_PUBLIC_WHATSAPP_NUMBER?.replace(/[^0-9]/g, "") || DEFAULT_SETTINGS.whatsapp_number.replace(/[^0-9]/g, "");
}
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
      <div style="background:#000;padding:24px;text-align:center">
        <h1 style="color:#86efac;margin:0;font-size:20px">${DEFAULT_SETTINGS.site_name}</h1>
      </div>
      <div style="padding:24px;background:#fbfbf5">
        ...
        <p style="margin-top:24px;font-size:13px;color:#666">This notification was sent automatically from ${DEFAULT_SETTINGS.site_name}.</p>
      </div>
    </div>
  `;
      ...
      <div style="padding:24px;background:#fbfbf5">
        ...
        <div style="border-top:1px solid #e0e0e0;padding-top:16px;margin-top:16px">
          <p style="font-size:12px;color:#999;line-height:1.5">
            ${DEFAULT_SETTINGS.site_name} | ${DEFAULT_SETTINGS.address_city}, India<br>
