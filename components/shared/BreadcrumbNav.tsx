...
export default function BreadcrumbNav({ items }: { items: { label: string; href: string }[] }) {
  return (
    <nav aria-label="Breadcrumb" className="bg-surface/80 border-b border-hairline-light overflow-x-auto">
      <div className="container-cinematic py-2.5 sm:py-3">
        <ol className="flex items-center gap-1.5 text-micro sm:text-caption text-shade-40 min-w-0">
          <li className="flex items-center gap-1.5 shrink-0">
            <Link href="/" className="hover:text-text transition-colors flex items-center gap-1" aria-label="Home">
            ...
            return (
              <li key={item.href} className="flex items-center gap-1.5 min-w-0">
                ...
                {isLast ? (
                  ...
                  <Link href={item.href} className="hover:text-text transition-colors truncate max-w-[100px] sm:max-w-none">{item.label}</Link>
                )}
              </li>
            );
          })}
